...
async function createDynamicEnvironment() {
  const server = (0, import_core.getInput)("server");
  const apiKey = (0, import_core.getInput)("api_key");
  const space = (0, import_core.getInput)("space");
  const name = (0, import_core.getInput)("environment_name");
  ...
  const environments = await httpClient.getJson(
    getEnvironmentsUrl,
    {
  ...
  if (!environments.result?.Items.find((environment) => environment.Name === name)) {
...
createDynamicEnvironment().catch((reason) => {
  (0, import_core.setFailed)(reason);
});
